    ...
    async execute(interaction) {
        try {
            if (interaction.isChatInputCommand()) {
                const logChannel = await (0, channelManager_1.getLogChannel)(interaction.guild, 'bot-komut');
                if (logChannel) {
                    await (0, logger_1.sendLog)(logChannel, '⚙️ Komut Kullanıldı', `${interaction.user} tarafından \`/${interaction.commandName}\` komutu kullanıldı.`, 0x3498db, [
