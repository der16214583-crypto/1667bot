    ...
    async execute(message) {
        try {
            ...
            if (adLinks.some(link => content.includes(link))) {
                ...
                const channel = await (0, channelManager_1.getLogChannel)(message.guild, 'reklam-log');
                await (0, logger_1.sendLog)(channel, '🚨 Reklam Engellendi', 'Bir reklam içeriği silindi', 0xff0000, [
            ...
            if (wordsInMessage.some(word => badWords.includes(word))) {
                await message.delete().catch(() => { });
                const channel = await (0, channelManager_1.getLogChannel)(message.guild, 'küfür-log');
                await (0, logger_1.sendLog)(channel, '🤬 Küfür Engellendi', 'Bir küfürlü mesaj silindi', 0xff0000, [
                    { name: 'Kullanıcı', value: `${message.author} (${message.author.id})`, inline: true },
                    { name: 'Kanal', value: `${message.channel}`, inline: true },
                    { name: 'Mesaj İçeriği', value: message.content, inline: false },
                ]);
            ...
            if (userData) {
                ...
                if (userData.count >= 5) {
                    ...
                    if (userData.count === 5) { // Sadece bir kez logla
                        const channel = await (0, channelManager_1.getLogChannel)(message.guild, 'spam-log');
                        await (0, logger_1.sendLog)(channel, '⚠️ Spam Algılandı', 'Bir kullanıcı kısa sürede çok fazla mesaj gönderdi', 0xffa500, [
                            { name: 'Kullanıcı', value: `${message.author} (${message.author.id})`, inline: true },
                            { name: 'Kanal', value: `${message.channel}`, inline: true },
                        ]);
                    }
